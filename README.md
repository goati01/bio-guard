...
本系統不採用傳統的線性濕度預警，而是透過動態非線性公式計算 **黴菌生長指數 (MGI)**：
...
> `MGI = round( (RH - 55)^1.28 * C_reg * C_mat * NDWI * Vent * 0.25 )`

### 2. 關鍵係數說明
* **地理修正 (`C_reg`)**：自動偵測點擊座標。針對「新竹/基隆風壓區」設定 1.45 ~ 1.55 之加權，反映外部濕冷空氣對牆體的滲透壓力。
* **材料敏感度 (`C_mat`)**：
